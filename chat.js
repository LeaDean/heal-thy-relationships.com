...
(function () {
    ...
    #ht-chat-fab {
        ...
        overflow: hidden;
        animation: ht-fab-pulse 3s ease-in-out infinite;
    }
    @keyframes ht-fab-pulse {
        0%, 100% { box-shadow: 0 8px 32px var(--c-cool-glow), 0 0 0 0 rgba(20,184,166,0.4); }
        50%       { box-shadow: 0 8px 32px var(--c-cool-glow), 0 0 0 10px rgba(20,184,166,0); }
    }
    #ht-chat-fab.open { animation: none; }
    #ht-chat-fab::after {
        content: '';
        position: absolute;
        inset: 0;
    ...
    @media (max-width: 480px) {
        #ht-chat-fab { bottom: 20px; right: 20px; width: 60px; height: 60px; }
        #ht-chat-nudge { bottom: 92px; right: 20px; max-width: 190px; font-size: 0.78rem; }
        #ht-chat-panel { bottom: 92px; right: 16px; left: 16px; width: auto; height: min(520px, calc(100dvh - 120px)); border-radius: 16px; }
        .ht-input-row { padding: 0.7rem 0.8rem 0.9rem; }
        .ht-input-row textarea { font-size: 16px; }
        .ht-messages { padding: 1rem 1rem 0.4rem; }
        .ht-panel-header { padding: 1rem 1.2rem 0.9rem; }
        .ht-bubble { font-size: 0.85rem; padding: 0.7rem 0.9rem; }
        .ht-suggestions { padding: 0.4rem 1rem 0.7rem; }
    }
    `;

    const SUGGESTIONS = [
        "How can I improve communication with my partner?",
        "I keep repeating the same patterns — can you help?",
...
Whether you're curious about how the work unfolds, wondering if this is the right fit, or ready to explore something specific, I'm glad you're here.
